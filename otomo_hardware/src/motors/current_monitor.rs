...
impl<C: Channel<ADC1, ID = u8>, D: Channel<ADC1, ID = u8>> CurrentMonitor<C, D> {
    ...
    pub fn get_currents(&mut self) -> (u16, u16) {
        ...
        let left_millivolts = self.adc.sample_to_millivolts(left_sample);
        let right_millivolts = self.adc.sample_to_millivolts(right_sample);

        (left_millivolts, right_millivolts)
    }
}
